...
<html lang="en">
  <head>
    <meta charset="utf-8">
    <title>Import - Create Portfolio Database</title>
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta name="description" content="">
    <meta name="author" content="">

    <link href="/cfPortfolio/admin/assets/css/bootstrap.min.css" rel="stylesheet">

    <link href="/cfPortfolio/admin/assets/css/bootstrap-responsive.min.css" rel="stylesheet">
...
<cfelse>
	...
	<cfset database = "learncfinaweektest" />
	...
	<cftry>
		...
		<cfoutput>
			...
			<cfelseif findnocase('windows',systemname)>
				...
				<cfsavecontent variable="batchCheck">cd C:\Program Files\MYSQL\MYSQL Server 5.7\bin
				mysql -u root -p#form.mysqlRoot# --execute="SHOW Databases" mysql
				</cfsavecontent>
				<cfsavecontent variable="batchContent">cd C:\Program Files\MYSQL\MYSQL Server 5.7\bin
				mysql -uroot -p#form.mysqlRoot# -e "CREATE DATABASE #database#"
				</cfsavecontent>		
			<cfelse>
			...
			<cfelse>
				...
				<cfif len(batchError)>
					<cfif findnocase('command not found',batchError)>
						...
						<cfset messages['database'].status = 'bad' />
						<cfset messages['database'].detailedMessage = 'Your copy of MySQL does not have the command line feature installed.  You must manually create  an empty database called portfolio and retry the process.  To learn how to create a database click <a href="http://www.learncfinaweek.com/install/createdatabase/">here</a>.' />
					<cfelse>
						<cfset messages['database'].message = 'Database could not be created' />
						<cfset messages['database'].status = 'bad' />
	...
	<cftry>	
		<cfscript>
			...
			if(structKeyExists(myObj.getDatasources(),database)){
				...
				myObj.setMYSQL5(driver="MySQL5", 
			        ...
			        description = "Portfolio sample database",
			        pooling = true,
		...
		<cftry>
			...
			<cfcatch type="any">
				...
				<cfset messages['tables'].detailedMessage = 'You must create a blank database called Portfolio.' />
		...
		<cfif connectionSuccessfull>
			...
			<cftry>
				...
				<cfif !listfindnocase(tableNames,'pm_users')>
					<cfquery datasource="#database#">
	CREATE  TABLE `#database#`.`pm_users` (
							  `ID` INT NOT NULL AUTO_INCREMENT ,
				...
				<cfif !listfindnocase(tableNames,'Carousel')>
					<cfquery datasource="#database#">
						CREATE  TABLE `#database#`.`Carousel` (
							  `ID` INT NOT NULL AUTO_INCREMENT ,
					...
					<cfif !listfindnocase(tableNames,'Document')>
					<cfquery datasource="#database#">
						    CREATE  TABLE `#database#`.`Document` (
							  `ID` INT NOT NULL AUTO_INCREMENT ,
					...
					<cfif !listfindnocase(tableNames,'Experience')>
					<cfquery datasource="#database#">
						    CREATE  TABLE `#database#`.`Experience` (
							  `ID` INT NOT NULL AUTO_INCREMENT ,
				...
				<cfif !listfindnocase(tableNames,'Mission')>
					<cfquery datasource="#database#">
						  
				    CREATE  TABLE `#database#`.`Mission` (
							  `ID` INT NOT NULL AUTO_INCREMENT ,
                              `MissionStatement` VARCHAR(5000) NOT NULL  ,
                               `Highlight1` VARCHAR(100) NOT NULL  ,
                                 `Highlight2` VARCHAR(100)  NULL  ,
					...
					<cfif !listfindnocase(tableNames,'Organization')>
					<cfquery datasource="#database#">
						  
			
   						 CREATE  TABLE `#database#`.`Organization` (
							  `ID` INT NOT NULL AUTO_INCREMENT ,
                              `OrganizationName` VARCHAR(100) NOT NULL  ,
                               `OrganizationPosition` VARCHAR(50) NOT NULL ,
                             `User_Id` int NOT NULL  ,
                              `Portfolio_Id` int NOT NULL ,
							  `CreateDate`  datetime NULL  ,
					...
					<cfif !listfindnocase(tableNames,'PM_Roles')>
					<cfquery datasource="#database#">
						  
			
   					    CREATE  TABLE `#database#`.`PM_Roles` (
							  `ID` INT NOT NULL AUTO_INCREMENT ,
					...
					<cfif !listfindnocase(tableNames,'Profile')>
					<cfquery datasource="#database#">
						  
			
   						
   				    CREATE  TABLE `#database#`.`Profile` (
							  `ID` INT NOT NULL AUTO_INCREMENT ,
                               `User_Id` int NOT NULL , 
					...
					<cfif !listfindnocase(tableNames,'Resumes')>
					<cfquery datasource="#database#">
						  
			 CREATE  TABLE `#database#`.`Resumes` (
							  `ID` INT NOT NULL AUTO_INCREMENT ,       
                               `ResumeName` VARCHAR(50) NOT NULL  ,
                               `ResumeLocation` VARCHAR(50) NOT NULL  ,
                                `CreateDate` datetime NOT NULL,
                                 `User_Id` int NOT NULL ,      
							  PRIMARY KEY (`ID`) );
   				 ...
   				 CREATE  TABLE `#database#`.`Portfolio` (
						...
						<cfif !listfindnocase(tableNames,'Skill')>
					<cfquery datasource="#database#">
						  
			    CREATE  TABLE `#database#`.`Skill` (
							  `ID` INT NOT NULL AUTO_INCREMENT ,       
				...
				<cfif !listfindnocase(tableNames,'portfolioweb')>
					<cfquery datasource="#database#">
						CREATE  TABLE `#database#`.`portfolioweb` (
							  `id` INT NOT NULL AUTO_INCREMENT ,
			...
			<cftry>
				...
				<cfquery name="skillCheck" datasource="#database#">
					SELECT
				...
				<cfquery name="portfoliowebCheck" datasource="#database#">
					SELECT
						count(id) as totalRows
					FROM
						portfolioWeb
